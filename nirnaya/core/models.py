...
class ParameterModel(BaseModel):
    ...
    type_spelling: str = Field(
        ..., description="The exact literal type spelling from C++ source"
    )
...
class FieldModel(BaseModel):
    ...
    is_public: bool = Field(
        ..., description="True if public; False if private or protected"
    )
...
class FunctionModel(BaseModel):
    ...
    name: str = Field(
        ..., description="Fully qualified or relative name of the function"
    )
    return_type: str = Field(..., description="The return type signature")
    parameters: list[ParameterModel] = Field(
        default_factory=list, description="Ordered parameter list"
    )
    is_virtual: bool = Field(default=False, description="True if marked virtual")
    is_const: bool = Field(
        default=False, description="True if marked const (for member functions)"
    )
...
class StructModel(BaseModel):
    ...
    methods: list[FunctionModel] = Field(
        default_factory=list, description="Tracked member functions"
    )
    is_class: bool = Field(
        ..., description="True if declared as 'class'; False if declared as 'struct'"
    )
    ...
    def sort_fields_by_offset(cls, v: list[FieldModel]) -> list[FieldModel]:
        ...
        if all(f.offset_bits is not None for f in v):
            return sorted(
                v, key=lambda x: x.offset_bits if x.offset_bits is not None else -1
            )
        return v
...
class EnumModel(BaseModel):
    ...
    values: dict[str, int] = Field(
        ..., description="Mapping of literal names to their underlying integer values"
    )
...
class TypedefModel(BaseModel):
    ...
    target_type: str = Field(
        ..., description="The original underlying type spelling being mapped to"
    )
...
class BlueprintModel(BaseModel):
    ...
    header_path: str = Field(
        ..., description="Relative file path to the C++ header being tracked"
    )
    captured_at: datetime = Field(
        ..., description="Timestamp of when this blueprint snapshot was generated"
    )
    nirnaya_version: str = Field(
        ..., description="Version of the Nirnaya tool used to run the extraction"
    )
...
class Violation(BaseModel):
    ...
    description: str = Field(
        ..., description="A human-readable breakdown explaining exactly what drifted"
    )
    old_value: str | None = Field(
        None, description="The structural state captured in the source baseline"
    )
    new_value: str | None = Field(
        None, description="The incoming configuration discovered in local adjustments"
    )
    line_number: int | None = Field(
        None, description="Line number of the violation location if trackable"
    )
    suggested_fix: str | None = Field(
        None, description="Actionable C++ refactoring tip to avoid consumer ABI crashes"
    )
