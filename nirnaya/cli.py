...
def _get_vault_or_exit(reporter: TerminalReporter) -> StorageVault:
    ...
    if not root:
        reporter.print_error("Nirnaya must be initialized inside a valid active Git repository workspace.")
        raise typer.Exit(code=2)
        
    vault = StorageVault(root)
    if not vault.vault_dir.exists():
        reporter.print_error("No active .nirnaya vault metadata vault discovered. Run 'nirnaya init' first.")
        raise typer.Exit(code=2)
    return vault
...
@app.command()
def init(
    headers: List[str] = typer.Argument(None, help="Target C++ interface headers to bind under tracking constraint."),
    project_name: str = typer.Option("mylib", "--name", "-n", help="Human-readable reference name tag for the project asset.")
):
    ...
    if not root:
        reporter.print_error("Could not find a valid Git root repository container boundary.")
        raise typer.Exit(code=2)

    vault = StorageVault.initialize(root, project_name)
    reporter.print_success(f"Initialized storage vault registry inside [cyan]{vault.vault_dir}[/cyan]")
    ...
    if headers:
        ...
        for h in headers:
            ...
            if not h_path.exists():
                reporter.print_warning(f"Header target file reference not found on disk: {h}")
                continue
            ...
            try:
                ...
                reporter.print_success(f"Captured immutable baseline profile snapshot for: [cyan]{h}[/cyan]")
            except Exception as e:
                reporter.print_error(f"Failed parsing target layout configuration {h}: {e}")

        reporter.print_success(f"Tracked and cataloged [bold]{len(headers)}[/bold] interface contracts.")


@app.command()
def check(
    header: Optional[str] = typer.Argument(None, help="Check an isolated file layout definition specifically."),
    no_color: bool = typer.Option(False, "--no-color", help="Mutes rich text coloring escape sequences completely.")
):
    ...
    if not headers_to_check:
        reporter.print_warning("No trackable contract configurations found. Register targets with 'nirnaya init <files>'")
    ...
    for h in headers_to_check:
        h_path = Path(h).resolve()
        if not h_path.exists():
            reporter.print_error(f"Tracked element link severed. File missing from layout: {h}")
        ...
        try:
            ...
            reporter.print_error(f"Missing historical baseline configuration trace for tracked asset: {h}")
            global_violations_found = True
            continue
    ...
    reporter.print_success("All public interface layout commitments verified perfectly.")


@app.command()
def update(
    header: str = typer.Argument(..., help="Target header contract baseline to explicitly increment.")
):
    ...
    if not h_path.exists():
        reporter.print_error(f"Cannot refresh target contract. File missing from disk path location: {header}")
        raise typer.Exit(code=2)
    ...
    try:
        ...
        reporter.print_success(f"Golden contract layout boundary incremented for: [cyan]{header}[/cyan]")
    except Exception as e:
